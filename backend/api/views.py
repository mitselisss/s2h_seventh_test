            ...
            if user.is_active == False:
                ...
                if user and check_password(password, user.password):
                    ...
                    if serializer.is_valid():
                       ...
                       return JsonResponse({'error': 'Serializer not valid'}, status=400) 
                else:
                    return JsonResponse({'error': 'Wrong credentials'}, status=400)
        except:
            return JsonResponse({'error': 'User does not exist. You must sign up first'}, status=400)
            ...
            try:
                ...
                try:
                    ...
                    if password == password2:
                        ...
                        data_dict = {'email': email, 'username': username, 'password': password, 'is_active': False}
                        ...
                        if serializer.is_valid():
                            ...
                            try:
                                ...
                                expiration_hours = 24
                                expiration_timestamp = int((datetime.now() + timedelta(hours=expiration_hours)).timestamp())
                                token = signer.sign(f'{refresh}:{expiration_timestamp}')
                                
                                email_context = {
                                    'username': user.username,
                                    'activation_link': f'http://localhost:3000/activationMessage/{token}',
                                    'support_email': 'support@switchtohealthy.eu',  # Replace with your support email
                                }
...
def Activate(request, token):
    ...
    try:
        ...
        expiration_datetime = datetime.fromtimestamp(int(expiration_timestamp))
        ...
        if datetime.now() <= expiration_datetime:
            ...
            return JsonResponse({'error': "Activation link has expired."}, status=400, safe=False)
...
@api_view(['POST'])
def ActivationEmail(request, email):
...
@api_view(['POST'])
def ResetPasswordEmail(request, email):
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                email_context = {
                    'username': user.username,
                    'resetpassword_link': f'http://localhost:3000/resetpassword/{token}',
                    'support_email': 'support@switchtohealthy.eu',  # Replace with your support email
                }
...
def ResetPasswordLink(request, token):
    ...
    try:
        ...
        # Check if the token has expired
        if datetime.now() <= expiration_datetime:
            return JsonResponse("Active Link", status=200, safe=False)
        else:
...
def IdUserProfile(request, pk):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return JsonResponse({'error': 'No userProfile related with this user'}, status=400)
        ...
        serializer = UpdateUserProfileSerializer(instance=userProfile, data=data_to_serialize)
        #print(request.data)
        #print(serializer)
        if serializer.is_valid():
            serializer.save()
        ...
        serializer = RegisterUserProfileSerializer(data = data_to_serialize)
